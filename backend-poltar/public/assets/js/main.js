...
// Jika belum terdefinisi di config.js, sediakan fallback
...
async function trackReport(event) {
    ...
    try {
        ...
        resultBox.innerHTML = `<p class="text-rose-400 text-xs">Tidak dapat terhubung ke server backend.</p>`;
    }
}
    ...
    try {
            ...
            container.innerHTML = result.data.map((item) => `
                <div class="person-card group">
                    <div class="w-24 h-24 mx-auto mb-3.5 rounded-full bg-slate-900 border-2 border-white/10 group-hover:border-rose-500/80 transition-colors flex items-center justify-center overflow-hidden shadow-lg">
                        ${item.image_path
                    ? `<img src="${BACKEND_BASE_URL}/storage/${item.image_path}" alt="${item.name}" class="w-full h-full object-cover">`
        ...
        container.innerHTML = `
            <div class="col-span-1 sm:col-span-2 md:col-span-3 text-center p-6 glass-card border-rose-900/30">
                <p class="text-rose-400 text-xs font-semibold mb-1">Koneksi backend belum terhubung</p>
                <p class="text-slate-400 text-[11px]">Gagal mengambil data dari server backend (${BACKEND_BASE_URL}). Pastikan server Railway aktif.</p>
            </div>
        `;
    }
}